/**
 * widget.addElement tool handler.
...
export interface WidgetAddElementParams {
  widgetPath: string;
  elementType: string;
  elementName: string;
  parentName?: string;
...
export async function widgetAddElement(
  bridge: WebSocketBridge,
  params: WidgetAddElementParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'widget_add_element', args: params },
    timestamp: Date.now(),
  };
  try {
