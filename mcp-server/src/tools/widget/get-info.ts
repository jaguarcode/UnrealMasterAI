/**
 * widget.getInfo tool handler.
 * Returns widget tree hierarchy, root widget type, and bindings count.
 */
import { v4 as uuidv4 } from 'uuid';
import type { WebSocketBridge } from '../../transport/websocket-bridge.js';
import type { McpToolResult } from '../editor/ping.js';

export interface WidgetGetInfoParams {
  widgetPath: string;
}

export async function widgetGetInfo(
  bridge: WebSocketBridge,
  params: WidgetGetInfoParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'widget_get_info', args: params },
    timestamp: Date.now(),
  };
  try {
