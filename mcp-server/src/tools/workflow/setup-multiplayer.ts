...
export interface WorkflowSetupMultiplayerParams {
  gameName: string;
  basePath: string;
  maxPlayers?: number;
}

export async function workflowSetupMultiplayer(
  bridge: WebSocketBridge,
  params: WorkflowSetupMultiplayerParams
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'workflow_setup_multiplayer', args: params },
    timestamp: Date.now(),
  };
  try {
