...
export interface WorkflowCreateInventorySystemParams {
  systemName: string;
  basePath: string;
  maxSlots?: number;
}

export async function workflowCreateInventorySystem(
  bridge: WebSocketBridge,
  params: WorkflowCreateInventorySystemParams
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'workflow_create_inventory_system', args: params },
    timestamp: Date.now(),
  };
  try {
