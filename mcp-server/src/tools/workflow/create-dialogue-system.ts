...
export interface WorkflowCreateDialogueSystemParams {
  systemName: string;
  basePath: string;
}

export async function workflowCreateDialogueSystem(
  bridge: WebSocketBridge,
  params: WorkflowCreateDialogueSystemParams
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'workflow_create_dialogue_system', args: params },
    timestamp: Date.now(),
  };
  try {
