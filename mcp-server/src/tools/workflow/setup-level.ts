...
export interface WorkflowSetupLevelParams {
  levelName: string;
  levelPath: string;
...
export async function workflowSetupLevel(
  bridge: WebSocketBridge,
  params: WorkflowSetupLevelParams
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'workflow_setup_level', args: params },
    timestamp: Date.now(),
  };
  try {
