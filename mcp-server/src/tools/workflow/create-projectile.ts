...
export interface WorkflowCreateProjectileParams {
...
export async function workflowCreateProjectile(
  bridge: WebSocketBridge,
  params: WorkflowCreateProjectileParams
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'workflow_create_projectile', args: params },
    timestamp: Date.now(),
  };
  try {
