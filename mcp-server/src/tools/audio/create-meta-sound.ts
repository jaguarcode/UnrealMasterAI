/**
 * audio.create-meta-sound tool handler.
...
export interface AudioCreateMetaSoundParams {
  assetName: string;
  assetPath: string;
}

export async function audioCreateMetaSound(
  bridge: WebSocketBridge,
  params: AudioCreateMetaSoundParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'audio_create_meta_sound', args: params },
    timestamp: Date.now(),
  };
  try {
