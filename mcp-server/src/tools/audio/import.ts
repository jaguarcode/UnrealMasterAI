/**
 * audio.import tool handler.
 * Import an audio file into the Unreal project.
 */
...
export async function audioImport(
  bridge: WebSocketBridge,
  params: AudioImportParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'audio_import', args: params },
    timestamp: Date.now(),
  };
  try {
