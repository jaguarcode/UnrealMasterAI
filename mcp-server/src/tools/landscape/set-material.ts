...
export interface LandscapeSetMaterialParams {
  landscapeName: string;
  materialPath: string;
}

export async function landscapeSetMaterial(
  bridge: WebSocketBridge,
  params: LandscapeSetMaterialParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'landscape_set_material', args: params },
    timestamp: Date.now(),
  };
  try {
