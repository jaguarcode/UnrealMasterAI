...
export interface LandscapeGetInfoParams {
  landscapeName?: string;
}

export async function landscapeGetInfo(
  bridge: WebSocketBridge,
  params: LandscapeGetInfoParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'landscape_get_info', args: params },
    timestamp: Date.now(),
  };
  try {
