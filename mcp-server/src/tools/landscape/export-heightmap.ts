...
export interface LandscapeExportHeightmapParams {
  landscapeName: string;
  exportPath: string;
}

export async function landscapeExportHeightmap(
  bridge: WebSocketBridge,
  params: LandscapeExportHeightmapParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'landscape_export_heightmap', args: params },
    timestamp: Date.now(),
  };
  try {
