...
export interface GameplayAddInputActionParams {
  actionName: string;
  key: string;
  shift?: boolean;
  ctrl?: boolean;
...
export async function gameplayAddInputAction(
  bridge: WebSocketBridge,
  params: GameplayAddInputActionParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'gameplay_add_input', args: params },
    timestamp: Date.now(),
  };
  try {
