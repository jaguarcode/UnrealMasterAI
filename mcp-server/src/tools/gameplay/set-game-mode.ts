...
export interface GameplaySetGameModeParams {
  gameModePath: string;
}

export async function gameplaySetGameMode(
  bridge: WebSocketBridge,
  params: GameplaySetGameModeParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'gameplay_set_mode', args: params },
    timestamp: Date.now(),
  };
  try {
