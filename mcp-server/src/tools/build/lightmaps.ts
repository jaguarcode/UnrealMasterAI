/**
 * build.lightmaps tool handler.
 * Builds lightmaps for the current level.
 */
import { v4 as uuidv4 } from 'uuid';
import type { WebSocketBridge } from '../../transport/websocket-bridge.js';
import type { McpToolResult } from '../editor/ping.js';

export interface BuildLightmapsParams {
  quality?: string;
}

export async function buildLightmaps(
  bridge: WebSocketBridge,
  params: BuildLightmapsParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'build_lightmaps', args: params },
    timestamp: Date.now(),
  };
  try {
