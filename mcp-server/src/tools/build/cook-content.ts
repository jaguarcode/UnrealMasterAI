/**
 * build.cookContent tool handler.
...
export interface BuildCookContentParams {
  platform?: string;
}

export async function buildCookContent(
  bridge: WebSocketBridge,
  params: BuildCookContentParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'build_cook', args: params },
    timestamp: Date.now(),
  };
  try {
