...
export interface CurveSetKeysParams {
  curvePath: string;
  keys: CurveKey[];
}

export async function curveSetKeys(
  bridge: WebSocketBridge,
  params: CurveSetKeysParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'curve_set_keys', args: params },
    timestamp: Date.now(),
  };
  try {
