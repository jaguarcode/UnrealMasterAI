...
export interface SequencerAddBindingParams {
  sequencePath: string;
  actorName: string;
  bindingType?: string;
}

export async function sequencerAddBinding(
  bridge: WebSocketBridge,
  params: SequencerAddBindingParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'sequencer_add_binding', args: params },
    timestamp: Date.now(),
  };
  try {
