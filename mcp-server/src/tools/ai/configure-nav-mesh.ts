...
export interface AiConfigureNavMeshParams {
  agentRadius?: number;
  agentHeight?: number;
  cellSize?: number;
}

export async function aiConfigureNavMesh(
  bridge: WebSocketBridge,
  params: AiConfigureNavMeshParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'ai_configure_nav_mesh', args: params },
    timestamp: Date.now(),
  };
  try {
