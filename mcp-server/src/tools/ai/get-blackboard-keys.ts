...
export interface AiGetBlackboardKeysParams {
  blackboardPath: string;
}

export async function aiGetBlackboardKeys(
  bridge: WebSocketBridge,
  params: AiGetBlackboardKeysParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'ai_get_blackboard_keys', args: params },
    timestamp: Date.now(),
  };
  try {
