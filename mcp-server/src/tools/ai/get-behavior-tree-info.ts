...
export interface AiGetBehaviorTreeInfoParams {
  treePath: string;
}

export async function aiGetBehaviorTreeInfo(
  bridge: WebSocketBridge,
  params: AiGetBehaviorTreeInfoParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'ai_get_behavior_tree_info', args: params },
    timestamp: Date.now(),
  };
  try {
