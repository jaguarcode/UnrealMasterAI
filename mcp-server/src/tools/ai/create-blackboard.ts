...
export interface AiCreateBlackboardParams {
  blackboardName: string;
  blackboardPath: string;
}

export async function aiCreateBlackboard(
  bridge: WebSocketBridge,
  params: AiCreateBlackboardParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'ai_create_blackboard', args: params },
    timestamp: Date.now(),
  };
  try {
