...
export interface AiAddBlackboardKeyParams {
  blackboardPath: string;
  keyName: string;
  keyType: string;
}

export async function aiAddBlackboardKey(
  bridge: WebSocketBridge,
  params: AiAddBlackboardKeyParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'ai_add_blackboard_key', args: params },
    timestamp: Date.now(),
  };
  try {
