/**
 * physics.getInfo tool handler.
 * Reads bodies, constraints, and profiles from a PhysicsAsset.
 */
import { v4 as uuidv4 } from 'uuid';
import type { WebSocketBridge } from '../../transport/websocket-bridge.js';
import type { McpToolResult } from '../editor/ping.js';

export interface PhysicsGetInfoParams {
  physicsAssetPath: string;
}

export async function physicsGetInfo(
  bridge: WebSocketBridge,
  params: PhysicsGetInfoParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'physics_get_info', args: params },
    timestamp: Date.now(),
  };
  try {
