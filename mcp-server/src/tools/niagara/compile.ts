...
export interface NiagaraCompileParams {
  systemPath: string;
}

export async function niagaraCompile(
  bridge: WebSocketBridge,
  params: NiagaraCompileParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'niagara_compile', args: params },
    timestamp: Date.now(),
  };
  try {
