/**
 * niagara.setParameter tool handler.
...
export interface NiagaraSetParameterParams {
  systemPath: string;
  parameterName: string;
  value: unknown;
}

export async function niagaraSetParameter(
  bridge: WebSocketBridge,
  params: NiagaraSetParameterParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'niagara_set_parameter', args: params },
    timestamp: Date.now(),
  };
  try {
