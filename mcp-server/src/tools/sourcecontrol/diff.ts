/**
 * sourcecontrol.diff tool handler.
...
export interface SourcecontrolDiffParams {
  assetPath: string;
}

export async function sourcecontrolDiff(
  bridge: WebSocketBridge,
  params: SourcecontrolDiffParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'sourcecontrol_diff', args: params },
    timestamp: Date.now(),
  };
  try {
