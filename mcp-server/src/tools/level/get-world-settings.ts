/**
 * level-getWorldSettings tool handler.
 * Retrieves world settings for the current level via the Python bridge.
 */
import { v4 as uuidv4 } from 'uuid';
import type { WebSocketBridge } from '../../transport/websocket-bridge.js';
import type { McpToolResult } from '../editor/ping.js';

export async function levelGetWorldSettings(bridge: WebSocketBridge): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'level_world_settings', args: {} },
    timestamp: Date.now(),
  };
  try {
