/**
 * level-open tool handler.
 * Opens an existing level in the Unreal editor via the Python bridge.
 */
import { v4 as uuidv4 } from 'uuid';
import type { WebSocketBridge } from '../../transport/websocket-bridge.js';
import type { McpToolResult } from '../editor/ping.js';

export interface LevelOpenParams {
  levelPath: string;
}

export async function levelOpen(
  bridge: WebSocketBridge,
  params: LevelOpenParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'level_open', args: params },
    timestamp: Date.now(),
  };
  try {
