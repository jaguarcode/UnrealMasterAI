/**
 * level-save tool handler.
 * Saves the current level in the Unreal editor via the Python bridge.
 */
import { v4 as uuidv4 } from 'uuid';
import type { WebSocketBridge } from '../../transport/websocket-bridge.js';
import type { McpToolResult } from '../editor/ping.js';

export async function levelSave(bridge: WebSocketBridge): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'level_save', args: {} },
    timestamp: Date.now(),
  };
  try {
