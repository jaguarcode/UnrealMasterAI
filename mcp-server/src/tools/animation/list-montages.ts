...
export interface AnimListMontagesParams {
  skeletonPath?: string;
}

export async function animListMontages(
  bridge: WebSocketBridge,
  params: AnimListMontagesParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'anim_list_montages', args: params },
    timestamp: Date.now(),
  };
  try {
