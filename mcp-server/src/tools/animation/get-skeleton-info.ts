...
export interface AnimGetSkeletonInfoParams {
  skeletonPath: string;
}

export async function animGetSkeletonInfo(
  bridge: WebSocketBridge,
  params: AnimGetSkeletonInfoParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'anim_skeleton_info', args: params },
    timestamp: Date.now(),
  };
  try {
