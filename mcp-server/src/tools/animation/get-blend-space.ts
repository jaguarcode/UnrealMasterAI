...
export interface AnimGetBlendSpaceParams {
  blendSpacePath: string;
}

export async function animGetBlendSpace(
  bridge: WebSocketBridge,
  params: AnimGetBlendSpaceParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'anim_blend_space', args: params },
    timestamp: Date.now(),
  };
  try {
