/**
 * asset.setMetadata tool handler.
...
export interface AssetSetMetadataParams {
  assetPath: string;
  key: string;
  value: string;
}

export async function assetSetMetadata(
  bridge: WebSocketBridge,
  params: AssetSetMetadataParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'asset_metadata', args: params },
    timestamp: Date.now(),
  };
  try {
