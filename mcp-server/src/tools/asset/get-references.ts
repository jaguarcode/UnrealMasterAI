/**
 * asset.getReferences tool handler.
...
export interface AssetGetReferencesParams {
  assetPath: string;
}

export async function assetGetReferences(
  bridge: WebSocketBridge,
  params: AssetGetReferencesParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'asset_references', args: params },
    timestamp: Date.now(),
  };
  try {
