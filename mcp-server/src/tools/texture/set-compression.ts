...
export interface TextureSetCompressionParams {
  texturePath: string;
  compressionType: string;
}

export async function textureSetCompression(
  bridge: WebSocketBridge,
  params: TextureSetCompressionParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'texture_set_compression', args: params },
    timestamp: Date.now(),
  };
  try {
