...
export interface TextureListTexturesParams {
  directory?: string;
  filter?: string;
}

export async function textureListTextures(
  bridge: WebSocketBridge,
  params: TextureListTexturesParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'texture_list_textures', args: params },
    timestamp: Date.now(),
  };
  try {
