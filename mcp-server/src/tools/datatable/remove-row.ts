...
export interface DatatableRemoveRowParams {
  tablePath: string;
  rowName: string;
}

export async function datatableRemoveRow(
  bridge: WebSocketBridge,
  params: DatatableRemoveRowParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'datatable_remove_row', args: params },
    timestamp: Date.now(),
  };
  try {
