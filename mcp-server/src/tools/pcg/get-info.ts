...
export interface PcgGetInfoParams {
  graphPath: string;
}

export async function pcgGetInfo(
  bridge: WebSocketBridge,
  params: PcgGetInfoParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'pcg_get_info', args: params },
    timestamp: Date.now(),
  };
  try {
