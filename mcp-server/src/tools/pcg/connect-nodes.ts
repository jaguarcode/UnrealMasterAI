/**
 * pcg.connectNodes tool handler.
 * Connects two nodes in a PCGGraph via their pins.
 */
import { v4 as uuidv4 } from 'uuid';
import type { WebSocketBridge } from '../../transport/websocket-bridge.js';
import type { McpToolResult } from '../editor/ping.js';

export interface PcgConnectNodesParams {
  graphPath: string;
  sourceNode: string;
  sourcePin: string;
  targetNode: string;
...
export async function pcgConnectNodes(
  bridge: WebSocketBridge,
  params: PcgConnectNodesParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'pcg_connect_nodes', args: params },
    timestamp: Date.now(),
  };
  try {
