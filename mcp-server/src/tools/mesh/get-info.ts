...
export interface MeshGetInfoParams {
  meshPath: string;
}

export async function meshGetInfo(
  bridge: WebSocketBridge,
  params: MeshGetInfoParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'mesh_info', args: params },
    timestamp: Date.now(),
  };
  try {
