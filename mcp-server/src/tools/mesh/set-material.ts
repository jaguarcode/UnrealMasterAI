/**
 * mesh.setMaterial tool handler.
 * Assigns a material to a mesh slot.
 */
import { v4 as uuidv4 } from 'uuid';
import type { WebSocketBridge } from '../../transport/websocket-bridge.js';
import type { McpToolResult } from '../editor/ping.js';

export interface MeshSetMaterialParams {
  meshPath: string;
  materialPath: string;
  slotIndex?: number;
}

export async function meshSetMaterial(
  bridge: WebSocketBridge,
  params: MeshSetMaterialParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'mesh_set_material', args: params },
    timestamp: Date.now(),
  };
  try {
