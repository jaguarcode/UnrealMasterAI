/**
 * worldpartition.createDataLayer tool handler.
...
export interface WorldPartitionCreateDataLayerParams {
  layerName: string;
  layerType?: string;
}

export async function worldpartitionCreateDataLayer(
  bridge: WebSocketBridge,
  params: WorldPartitionCreateDataLayerParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'worldpartition_create_data_layer', args: params },
    timestamp: Date.now(),
  };
  try {
