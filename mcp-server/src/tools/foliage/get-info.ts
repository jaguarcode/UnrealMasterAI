...
export interface FoliageGetInfoParams {
  foliageTypePath: string;
}

export async function foliageGetInfo(
  bridge: WebSocketBridge,
  params: FoliageGetInfoParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'foliage_get_info', args: params },
    timestamp: Date.now(),
  };
  try {
