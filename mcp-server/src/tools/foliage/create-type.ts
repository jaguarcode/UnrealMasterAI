/**
 * foliage.createType tool handler.
...
export interface FoliageCreateTypeParams {
  typeName: string;
  typePath: string;
  meshPath: string;
}

export async function foliageCreateType(
  bridge: WebSocketBridge,
  params: FoliageCreateTypeParams,
): Promise<McpToolResult> {
  const msg = {
    id: uuidv4(),
    method: 'python.execute' as const,
    params: { script: 'foliage_create_type', args: params },
    timestamp: Date.now(),
  };
  try {
